...
- 首稿字数 > `hardMax` → 触发压缩模式
- 首稿字数 < `softMin`，且 PRE_WRITE_CHECK 未声明"允许偏短" → 触发扩写模式
...
- 修正后字数仍在 soft range 之外时：报 warning，但不再次递归（最多修正一次）
...
if originalCount > hardMax:
    mode = "compress"
elif originalCount < softMin and not PRE_WRITE_CHECK.allowShort:
    mode = "expand"
else:
    mode = "none"（跳过）
