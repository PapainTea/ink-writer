...
- 分流的根因判定参见上表第 3 列；塌方 case 的禁行路径（不得走 6.3 扩写掩盖情节不足）由下条"coverage 诚信律"保障

**⚠️ coverage 诚信律**（v0.1.16 关键）：`BEAT_SHEET_COVERAGE` 是 Writer 自报，**是上表路径分流的唯一依据**。**100% 的判定标准必须严格**——每场景"主要事件清单"所有条目都要在正文找到**可观察的对应段落**（按 Step 5 写作方式条 3 定义），不得松散解释为"事件大致提到了"。若 Writer 用 loose 解释把塌方 case 都判为 5/5，会让 6.3b 推剧情路径变死代码 → **首稿 < 88% 时，Beat Sheet 每场景必须做 line-by-line 对照**，只要有一条事件未落地就把该场景记为未命中，整章 coverage 不得为 100%。

### 6.3 扩写循环（描写薄路径）

**前置条件**（详细分流见 §6.2 表，本节只摘要）：上表第 2、3 行的两种情况——描写单薄或描写压缩。首稿 < plotThreshold 且 coverage < 100%（情节容量不足）走 6.3b，不走本节。
