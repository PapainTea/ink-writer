...
### 场景级字数预算（v0.1.15 由 Step 3 Beat Sheet 承接，此处仅复核）

Step 3 的 Beat Sheet 已经给每个场景填好 **6 列**（场景概述 / 主要事件 / 关键对话 / 收尾钩子 / 预算字数）。Step 4 在此复核：
...
**不要在此重复 Step 3 的 Beat Sheet**，只在 PRE_WRITE_CHECK 表格末尾注明 "Beat Sheet 就位 = 是"。

**写作时对照**：Writer 在 Step 5 写每个场景时逐条对照 Beat Sheet 的"主要事件"，**全部命中**（3/5 条清单全部要在正文能找到对应实现，一条都不能漏）。
...
- **target**：目标字数（由作者 / book_rules 设置，例 4500）
- **plotThreshold = target × (1 − plotThresholdPct%)**（默认 plotThresholdPct=12，即 target×0.88；< 此值判定"情节容量不足"走 Step 6.3b）
  - **12% 选值依据**：经验上 Writer 首稿 ≥ 88% 时缺口可靠补细节解决（2-3 段铺陈 / 感官 / 反应即可），< 88% 时多半是场景量缺 1 个，补细节只会稀释密度。该值可在 `book_rules.yaml.length.plotThresholdPct` 覆盖
- **hardGate = target**（最终硬门槛，等于 target 本身，无任何百分比折扣；最终字数必须 ≥ target 才允许过 Step 6）
- **hardMax = target × (1 + hardMaxPct%)**（默认 hardMaxPct=20，即 target×1.2；超过触发压缩。保留百分比是因为超长的"手感"与 target 非线性——target 翻倍的章节超长容忍度不应也翻倍）
- **countingMode**：`zh_chars`（按 CJK 字符数）或 `en_words`

**写作策略**：瞄准 target，**最终字数必须 ≥ target**（无软门槛、无百分比容忍）。偏离处理见 §04 Step 6。
