...
**particle_ledger.md 初始化（全书种都要）**：表头 + `| 0 | - | 0 | 0 | 0 | 开书初始 | init-0 |` 初始行。数值型体裁的初始行记录的是硬资源（金钱 / 灵石 / 装备 / 等级等），非数值型体裁记录软资源（人情债 / 合作链 / 情报权 / 信任值等）；两者的 schema 完全一致，只是 `book_rules.md` 的 `resourceTypes` 配置不同。
...
**关于资源账本 particle_ledger.md**：**全部书种都要**，不是只数值型体裁。非数值型体裁（都市、科幻、治愈等）同样有**人情债 / 合作链 / 情报权**等需要追踪的"软资源"，都走 ledger。数值型和非数值型的差异在于 ledger 里记录的**资源类型**（由 `book_rules.md` 的 `numericalSystemOverrides.resourceTypes` 决定），不是"有没有 ledger"。
