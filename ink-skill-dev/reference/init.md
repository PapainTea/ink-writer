...
> 2. 看一眼 `story/book_rules.md` 的 length 配置（target 字数 + hardMax），默认 target 4500 字、hardGate = target 本身无折扣，想改现在说
> 3. 准备好就说『写第 1 章』
>
...
length:                                              # 字数配置（v0.1.14 canonical，详见 §04 / §09）
  target: 4500                                         # 目标字数；hardGate = target 本身（最终硬门槛，无折扣、无旁路）
