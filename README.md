...
这一步会在 `~/novels/` 下创建 `books/` 子目录、复制最新 `CLAUDE.md` 进去、写入 `.ink-writer.yaml` 记录绝对路径。
...
**升级 instruction**：`git pull` 后在同一父目录重跑 `install.py`，`books/CLAUDE.md` 会刷新为最新；已有的 `.ink-writer.yaml` 和书目录不受影响。
...
- ✅ 拼接生成器（generate.py）
- ✅ Claude Code 版（dist/CLAUDE.md，2942 行）
